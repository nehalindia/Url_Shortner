    ...
    //   }
        ...
        if(check){
           ...
           // return res.status(200).send({status:true, data:check })
            ...
            await SET_ASYNC(`${check.urlCode}`, JSON.stringify(check.longUrl))      ///dekh lenge stringify use krna hai ya nhi
            return res.status(200).send({status:true, data:check })
        }
            ...
            await SET_ASYNC(`${result.urlCode}`, JSON.stringify(result.longUrl))
